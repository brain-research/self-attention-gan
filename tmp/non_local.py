...
def sn_conv1x1(input_, output_dim, update_collection,
  ...
  with tf.variable_scope(name):
    ...
    w = tf.get_variable(
        ...
        initializer=init)
    w_bar = sn_ops.spectral_normed_weight(
        w, num_iters=1, update_collection=update_collection)
...
def sn_non_local_block_sim(x, update_collection, name,
  ...
  with tf.variable_scope(name):
    ...
    theta = sn_conv1x1(
        x, num_channels // 8, update_collection, init, 'sn_conv_theta')
    theta = tf.reshape(
        theta, [batch_size, location_num, num_channels // 8])

    phi = sn_conv1x1(
        x, num_channels // 8, update_collection, init, 'sn_conv_phi')
    ...
    attn_g = sn_conv1x1(
        attn_g, num_channels, update_collection, init, 'sn_conv_attn')
